...
- **只收已启用(`enabled`)、处理完成(`status === 'ready'`)且有播放地址的条目**;停用、处理中、失败的不进列表。
...
- **`size` 的来源**:处理完成时即记录(本地上传 = 转码产物大小;URL 导入 = 下载样本的真实大小),存量远程条目缺值时由服务端后台发一次 HEAD 请求补录落盘,首次请求返回 `null`、稍后即有。
